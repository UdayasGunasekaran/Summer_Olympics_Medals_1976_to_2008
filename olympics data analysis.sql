SELECT * FROM netflix.summer_olympic_medals_1976_to_2008;
...
WHERE City IS NULL OR Year IS NULL OR Sport IS NULL OR Discipline IS NULL OR Event IS NULL OR Athlete IS NULL OR Gender IS NULL OR Country_Code IS NULL OR Country IS NULL OR Event_gender IS NULL OR Medal IS NULL;


-- Identifying Duplicate rows

SELECT City, Year, Sport, Discipline, Event, Athlete, Gender, Country_Code, Country, Event_gender, Medal, COUNT(*) AS count
FROM netflix.summer_olympic_medals_1976_to_2008
...
WHERE (City, Year, Sport, Discipline, Event, Athlete, Gender, Country_Code, Country, Event_gender, Medal) NOT IN (
...
SELECT Gender, COUNT(Gender) AS Distribution, ROUND(COUNT(Gender) * 100 / (SELECT COUNT(Gender) FROM netflix.summer_olympic_medals_1976_to_2008)) AS Percentage
FROM netflix.summer_olympic_medals_1976_to_2008
GROUP BY Gender
...
SELECT Gender, Medal, COUNT(Medal) AS Distribution, ROUND(COUNT(Medal) * 100 / (SELECT COUNT(Medal) FROM netflix.summer_olympic_medals_1976_to_2008)) AS Percentage
FROM netflix.summer_olympic_medals_1976_to_2008
...
SELECT distinct Event, Count(Event) AS Distinct_Event_Count
FROM netflix.summer_olympic_medals_1976_to_2008
GROUP BY Event
...
SELECT DISTINCT City, Year, COUNT(City) AS Max_Hostings
FROM netflix.summer_olympic_medals_1976_to_2008
GROUP BY City, Year
ORDER BY Year ASC;
...
SELECT DISTINCT City, COUNT(Event) AS Events_Count
FROM netflix.summer_olympic_medals_1976_to_2008
GROUP BY City
...
SELECT DISTINCT Country, Medal_Count, Year
FROM (
...
SELECT Country, Sport, COUNT(Medal) AS Total_Medals
FROM netflix.summer_olympic_medals_1976_to_2008
